...
-- it means for every customer_id in the orders table, show the customer_id and the number in that city.


-- number 3
aprende_db=# select customer_id, count(*) as total_count, sum(total_price) as total_price, avg(total_price) as avg_price from orders group by customer_id;
